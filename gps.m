...
%                 
%                 n = 0;
%                 while (Obj.ser.BytesAvailable == 0)
%                     pause(0.0005);
%                     n = n +1;
%                     if n >= 80
%                         
...
%                 end
%                 
%              cmMonObj.update([sprintf('%i Bytes',Obj.ser.BytesAvailable)]);   
%                % [~, errors] = Obj.read_command(cmMonObj,cmd);
% %                 if ~errors && strcmp(raw,raw_out)
% %                     errors = 0;
% %                     cmMonObj.update('this actually fucking happend wtf')
...
%             if Obj.ser.BytesAvailable > 0
%                 %for i = 1:5
%                     [data,errors] = Obj.read_command(cmMonObj,cmd);
%                     if ~errors
%                         cmMonObj.update(data);
%                         cmMonObj.update('geegee');
% 
%                         %break;                
%                     end
%                     pause(0.05);
...
%                            
            ...
            %[~,data,errors] = scan_for_cmd(char_array,'4861');
        ...
        function position_message(Obj,cmMonObj)
            ...
            %cmMonObj.update([sprintf('The date is: %s',Obj.date)]);
            %cmMonObj.update([sprintf('The time is: %s',Obj.time)]);
            ...
            %cmMonObj.update([sprintf('Altitude: %s m',num2str(Obj.height/100))]);
            
            %Satellites
            Obj.satVisible = hex2dec(data(103:104));
            Obj.satTracked = hex2dec(data(105:106));
            
            %cmMonObj.update([sprintf('Satellites Visible: %s',num2str(Obj.satVisible))]);
            %cmMonObj.update([sprintf('Satellites Tracked: %s',num2str(Obj.satTracked))]);
        end 
...
%             if strcmp(data,'03')
%                 cmMonObj.update('GPS entered Survey Mode!');
%                 cmMonObj.update([sprintf('Starting Latitude: %s',num2str(Obj.latread))]);
%                 cmMonObj.update([sprintf('Starting Long: %s',num2str(Obj.longread))]);
%                 
%                 
%             else
%                 cmMonObj.update('Survey Mode Error!');
%             end
        end
        
        function [data,errors] = savePos(Obj, cmMonObj)
            %fin = fopen('gps_info.cfg');
            fid = fopen('gps_info.cfg', 'w');
            Obj.pos_hold_long = Obj.long;
            Obj.pos_hold_lat = Obj.lat;
            Obj.pos_hold_alt = Obj.height;
            fwrite(fid, ['Time=' Obj.time double(sprintf('\n')) 'Date=' Obj.date double(sprintf('\n')) 'Current_Lat=' num2str(Obj.lat) double(sprintf('\n')) 'Current_Long=' num2str(Obj.long) double(sprintf('\n')) 'Current_Alt=' num2str(Obj.height) double(sprintf('\n')) 'Pos_Hold_Lat=' num2str(Obj.pos_hold_lat) double(sprintf('\n')) 'Pos_Hold_Long=' num2str(Obj.pos_hold_long) double(sprintf('\n')) 'Pos_Hold_Alt=' num2str(Obj.pos_hold_alt)]);
        ...
        function pos_hold_config(Obj)
             ...
             while 1
                ...
                if strfind(tline, 'Pos_Hold_Lat=')>0;
                    pos_hold_lati = tline(14:end);
                    Obj.pos_hold_lat = str2num(pos_hold_lati)
                end
                if strfind(tline, 'Pos_Hold_Long=')>0;
                    pos_hold_longi = tline(15:end);
                    Obj.pos_hold_long = str2num(pos_hold_longi);
                end    
                 if strfind(tline, 'Pos_Hold_Alt=')>0;
                    pos_hold_alti = tline(14:end);
    ...
    methods (Static)
%         %function [chksum] = compute_chksum(chksum_str)           
%             chksum = bitxor(hex2dec(chksum_str(1:2)),hex2dec(chksum_str(3:4))); % check data parity
%             for i = 6:2:length(chksum_str)
%                 chksum = bitxor(hex2dec(chksum_str(i-1:i)),chksum);
%             end
%         end
